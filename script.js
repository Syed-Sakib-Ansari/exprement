...
function openModal(id) {
    ...
    if (actualVideoContainer) {
        ...
        const needsNewIframe = !isSameMovie || !existingIframe || existingIframe.dataset.src !== url;
        if (needsNewIframe) {
            // 👑 🎯 FIXED: INTERNALLY INSULATED SRCDOC WRAPPER TO PREVENT THE VIDEO IFRAME FROM POLLUTING THE TOP TIMELINE STACK
            actualVideoContainer.innerHTML = `<iframe id="videoIframe" data-src="${url}" class="w-full h-full border-0 outline-none rounded-lg bg-black" srcdoc="<!DOCTYPE html><html><head><style>html,body{margin:0;padding:0;width:100%;height:100%;background:#000;overflow:hidden;}</style></head><body><iframe src='${url}' style='width:100%;height:100%;border:0;outline:none;' allowfullscreen='true' allow='autoplay; fullscreen; encrypted-media; picture-in-picture'></iframe></body></html>" frameborder="0" scrolling="no" marginwidth="0" marginheight="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share; fullscreen" allowfullscreen="true" webkitallowfullscreen="true" mozallowfullscreen="true"></iframe>`;
        }
    }
...
function playEpisode(index, btnElement) {
    ...
    if (actualVideo) {
        ...
        if (!existingIframe || existingIframe.dataset.src !== url) {
            // 👑 🎯 FIXED: INJECT SUB-EPISODES INSIDE IMMUNE REFERRER MEMORY SHIELDS
            actualVideo.innerHTML = `<iframe id="videoIframe" data-src="${url}" class="w-full h-full border-0 outline-none rounded-lg bg-black" srcdoc="<!DOCTYPE html><html><head><style>html,body{margin:0;padding:0;width:100%;height:100%;background:#000;overflow:hidden;}</style></head><body><iframe src='${url}' style='width:100%;height:100%;border:0;outline:none;' allowfullscreen='true' allow='autoplay; fullscreen; encrypted-media; picture-in-picture'></iframe></body></html>" frameborder="0" scrolling="no" marginwidth="0" marginheight="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share; fullscreen" allowfullscreen="true" webkitallowfullscreen="true" mozallowfullscreen="true"></iframe>`;
        }
    }
...
window.addEventListener('popstate', (event) => {
    ...
    if (modal && (!modal.classList.contains('hidden') || isModalClosing)) {
        if (!state || !state.isModalOpen) { 
            if (!isModalClosing) closeModal(false, false); 
...
// 👑 🎯 FIXED: FULL REFERRER IMMUNITY FOR ALL BROWSERS (Steals focus check to prevent stack overrides)
