...
function injectAdsterra(container, key, w, h) {
    ...
    setTimeout(() => {
        try {
            ...
            doc.open();
            doc.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><style>body{margin:0;padding:0;background:transparent;display:flex;justify-content:center;align-items:center;}</style></head><body><script type="text/javascript">atOptions = {'key' : '${key}','format' : 'iframe','height' : ${h},'width' : ${w},'params' : {}};</script><script type="text/javascript" src="https://onsetcab.com/${key}/invoke.js"></script></body></html>`);
            doc.close();
        } catch(e) { }
    }, 10);
}
...
async function loadContentDatabase() {
    try {
        ...
        console.warn("External JSON database failed to load.", err);
    } finally {
...
function toggleCategoryMenu(show, triggerBack = true) {
    ...
    if (show) {
        ...
        if (triggerBack && window.history.state?.isMenuOpen) window.history.back();
...
function dragStart(e) {
    ...
    fab.style.transition = 'none'; fab.setPointerCapture(e.pointerId);
}
function drag(e) {
    ...
    if (moved) {
        let nextX = initialX + dx; let nextY = initialY + dy;
        ...
        nextX = Math.max(0, Math.min(nextX, maxX)); nextY = Math.max(0, Math.min(nextY, maxY));
        translateX = nextX - initialX; translateY = nextY - initialY;
        fab.style.transform = `translate3d(${translateX}px, ${translateY}px, 0)`;
    }
}
// [UX: ভিডিও মডেল ক্লোজ হলে এবং ব্যাক নেভিগেশন হলে স্ক্রল রিস্টোর ঠিক রাখা]
function dragEnd(e) {
    if (!isDragging) return;
    isDragging = false; fab.releasePointerCapture(e.pointerId);
    if (moved) {
        let newX = initialX + translateX; let newY = initialY + translateY;
        const maxX = document.documentElement.clientWidth - fab.offsetWidth;
        const maxY = document.documentElement.clientHeight - fab.offsetHeight;
        newX = Math.max(0, Math.min(newX, maxX)); newY = Math.max(0, Math.min(newY, maxY));
        fab.style.transform = 'none'; fab.style.left = `${newX}px`; fab.style.top = `${newY}px`;
...
if (fab) {
    fab.addEventListener('pointerdown', dragStart); fab.addEventListener('pointermove', drag);
    fab.addEventListener('pointerup', dragEnd); fab.addEventListener('pointercancel', dragEnd);
...
window.addEventListener('resize', () => {
    ...
    const maxX = document.documentElement.clientWidth - fab.offsetWidth;
    const maxY = document.documentElement.clientHeight - fab.offsetHeight;
    if (fab.offsetLeft > maxX) fab.style.left = `${maxX}px`;
    if (fab.offsetTop > maxY) fab.style.top = `${maxY}px`;
});
...
function initHeroSlider() {
    ...
    let currentSlide = 0; sliderWrapper.innerHTML = ''; sliderDots.innerHTML = '';
    const isMobile = window.innerWidth <= 768; const sliderImageWidth = isMobile ? 600 : 1200;
    ...
    window.goToSlide = (index) => {
        const allSlides = document.querySelectorAll('.slide'); const allDots = sliderDots.children;
        if (allSlides.length > 0) {
            ...
            currentSlide = index; allSlides[currentSlide].classList.add('active');
...
function updateSearchUI() {
    ...
    if (searchInput.value.trim().length > 0) {
        searchIcon.classList.remove('fa-search'); searchIcon.classList.add('fa-times', 'cursor-pointer');
        if (libraryFilters) libraryFilters.style.display = 'none';
    } else {
        searchIcon.classList.remove('fa-times', 'cursor-pointer'); searchIcon.classList.add('fa-search');
        if (libraryFilters) libraryFilters.style.display = '';
    }
}
function handleSearchIconClick() { if (searchInput.value.trim().length > 0) clearSearch(); else searchInput.blur(); }

function clearSearch(preventRestore = false) {
    searchInput.value = ''; updateSearchUI(); searchInput.blur();
    if (!preventRestore && preSearchState) {
        ...
        preSearchState = null; initLibraryRender();
    }
}
function updateCanonical(url) { if (document.getElementById('canonicalLink')) document.getElementById('canonicalLink').setAttribute('href', 'https://moviedakhi.com/'); }

function switchView(viewName, filterCategory = null, mode = true, restoredCount = 0, targetScroll = 0) {
    if (mode) {
        const currentScroll = window.scrollY;
        const currentState = window.history.state || { view: currentView, category: null, displayedCount: libraryDisplayedCount, validDakhiState: true };
        try { window.history.replaceState({ ...currentState, scrollY: currentScroll }, ''); } catch (e) { }
    }
    currentView = viewName; homeView.classList.remove('active'); libraryView.classList.remove('active');
    if (viewName === 'home') {
        homeView.classList.add('active'); document.title = "MovieDakhi | Watch Free Movies & Web Series Online";
    } else {
        libraryView.classList.add('active');
        if (filterCategory) { searchInput.value = ''; updateSearchUI(); preSearchState = null; }
    ...
    if (mode) {
        try {
            ...
            if (!isBlob) {
                const url = new URL(window.location); url.searchParams.set('view', viewName);
                if (filterCategory && filterCategory !== 'all' && viewName === 'library') url.searchParams.set('category', filterCategory);
                else url.searchParams.delete('category');
                url.searchParams.delete('movie');
                if (mode === 'replace') window.history.replaceState(stateObj, '', url); else window.history.pushState(stateObj, '', url);
                updateCanonical(url.href);
            } else {
                if (mode === 'replace') window.history.replaceState(stateObj, '', window.location.href); else window.history.pushState(stateObj, '', window.location.href);
            }
            void document.documentElement.offsetHeight; window.scrollTo({ top: targetScroll, behavior: 'instant' });
        } catch (e) {
            void document.documentElement.offsetHeight; window.scrollTo({ top: targetScroll, behavior: 'instant' });
        }
    }
...
function createMovieCard(item) {
    const card = document.createElement('a'); const movieSlug = item.slug || generateMovieSlug(item.title);
    card.href = `${movieSlug}.html`; card.className = 'movie-card relative flex flex-col group cursor-pointer no-underline';
    ...
    card.innerHTML = `
        <div class="relative rounded-lg overflow-hidden bg-[#111] shadow-xl aspect-[2/3] ring-1 ring-white/5 md:ring-0 transition-all duration-300 group-hover:ring-white/20 md:group-hover:ring-transparent">
            ${qualityBadgeHtml} ${languageBadgeHtml}
    ...
    card.onclick = (e) => { e.preventDefault(); openModal(item.id); };
    return card;
}

function renderRecentAdds() {
    if (!recentAddsGrid) return; recentAddsGrid.innerHTML = '';
    const recentItems = contentData.filter(item => item.category === "Recent Adds"); const fragment = document.createDocumentFragment();
    recentItems.slice(0, 18).forEach((item, index) => {
        fragment.appendChild(createMovieCard(item)); let currentIdx = index + 1;
...
function renderCategorySections(forceRenderAll = false) {
    if (!categorySections) return; categorySections.innerHTML = ''; const fragment = document.createDocumentFragment();
    const sectionObserver = new IntersectionObserver((entries, observer) => {
        entries.forEach(entry => { if (entry.isIntersecting) { loadCategorySection(entry.target); observer.unobserve(entry.target); } });
    }, { root: null, rootMargin: '2500px 0px', threshold: 0.01 });

    function loadCategorySection(targetSection) {
        const cat = targetSection.getAttribute('data-category-lazy'); const lazyGrid = targetSection.querySelector('.lazy-grid'); const filtered = contentData.filter(m => m.category === cat);
        lazyGrid.innerHTML = ''; const cardsFragment = document.createDocumentFragment();
        filtered.slice(0, 11).forEach((item, index) => {
            cardsFragment.appendChild(createMovieCard(item)); let currentIdx = index + 1;
        ...
        const displayName = cat === 'Korean Country' ? 'Korean' : cat; const viewAllCard = document.createElement('div');
        ...
        viewAllCard.innerHTML = `<div class="flex flex-col items-center justify-center transition-transform duration-300 group-hover:scale-110"><div class="w-14 h-14 rounded-full bg-white/10 border border-white/20 flex items-center justify-center mb-4 group-hover:bg-red-600 group-hover:border-red-600 transition-all shadow-lg transition-transform duration-300 group-hover:scale-110"><i class="fas fa-arrow-right text-white text-xl"></i></div><h4 class="font-black text-sm uppercase text-white tracking-widest transition-transform duration-300 group-hover:scale-110">View All</h4><p class="text-[10px] text-gray-500 font-bold mt-2 uppercase tracking-tighter transition-transform duration-300 group-hover:scale-110">${displayName}</p></div>`;
        viewAllCard.onclick = () => { clearSearch(true); switchView('library', cat); }; lazyGrid.appendChild(viewAllCard);
        targetSection.querySelectorAll('.cat-ad-adsterra').forEach(container => { if (!container.dataset.loaded) { injectResponsiveAdNode(container); container.dataset.loaded = 'true'; } });
        targetSection.classList.remove('opacity-0'); targetSection.classList.add('opacity-100');
    }
    categories.filter(c => c !== 'all').forEach(cat => {
        if (contentData.filter(m => m.category === cat).length === 0) return;
        const displayName = cat === 'Korean Country' ? 'Korean' : cat; const section = document.createElement('section');
        section.className = 'mb-16 lazy-section opacity-0 min-h-[350px] transition-opacity duration-500'; section.setAttribute('data-category-lazy', cat);
        section.innerHTML = `<div class="w-full flex flex-col items-center gap-4 mb-8 mt-4 min-h-[150px]"><div class="cat-ad-adsterra w-full"></div><div class="cat-ad-adsterra w-full"></div></div><div class="flex items-center space-x-3 mb-8 justify-center"><div class="w-1.5 h-7 bg-red-600 rounded-full shadow-lg shadow-red-600/20"></div><h3 class="text-2xl md:text-5xl font-black tracking-tighter uppercase">${displayName}</h3></div><div class="lazy-grid grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-6 md:gap-8 justify-center max-w-10xl mx-auto"><div class="col-span-full py-16 flex justify-center items-center"><div class="w-8 h-8 border-4 border-red-600 border-t-transparent rounded-full animate-spin opacity-45"></div></div></div>`;
        if (forceRenderAll) loadCategorySection(section); else sectionObserver.observe(section); fragment.appendChild(section);
    });
    categorySections.appendChild(fragment);
}

function initLibraryRender(filter = "all", initialCount = 0) {
    if (!libraryGrid) return; const rawQuery = searchInput ? searchInput.value : '';
    const cleanStr = (str) => str ? str.toLowerCase().replace(/[^a-z0-9]/g, '') : ""; const cleanQuery = cleanStr(rawQuery); const isSearch = rawQuery.trim().length > 0;
    if (!libraryGrid.dataset.initialized) { libraryGrid.dataset.originalClasses = libraryGrid.className; libraryGrid.className = 'relative w-full'; libraryGrid.dataset.initialized = 'true'; }
    Array.from(libraryGrid.children).forEach(child => child.classList.add('hidden'));
    const gridId = isSearch ? 'subgrid-search' : `subgrid-${filter.replace(/\s+/g, '-')}`; activeSubGridId = gridId;
    let subGrid = document.getElementById(gridId); if (isSearch && subGrid) { subGrid.remove(); subGrid = null; }
    if (subGrid) {
        subGrid.classList.remove('hidden'); libraryDisplayedCount = parseInt(subGrid.dataset.displayedCount || ITEMS_PER_PAGE, 10);
        libraryData = contentData.filter(item => filter === "all" || item.category === filter || (filter === "all" && item.category === "Recent Adds"));
        updateLoadMoreVisibility(); return; 
    }
    subGrid = document.createElement('div'); subGrid.id = gridId; subGrid.className = libraryGrid.dataset.originalClasses; libraryGrid.appendChild(subGrid);
    ...
    libraryDisplayedCount = initialCount > 0 ? initialCount : ITEMS_PER_PAGE; subGrid.dataset.displayedCount = libraryDisplayedCount;
    ...
    if (isSearch && libraryView) window.scrollTo({ top: libraryView.offsetTop - 100, behavior: 'smooth' });
}

function renderLibraryChunk() {
    if (isLoading) return; isLoading = true; const nextCount = libraryDisplayedCount + ITEMS_PER_PAGE; const chunk = libraryData.slice(libraryDisplayedCount, nextCount);
    let subGrid = document.getElementById(activeSubGridId);
    if (chunk.length > 0 && subGrid) {
        const fragment = document.createDocumentFragment();
        chunk.forEach((item, index) => {
            fragment.appendChild(createMovieCard(item)); let currentIdx = libraryDisplayedCount + index + 1;
            if (currentIdx % 4 === 0) fragment.appendChild(createMobileNativeAdBlock());
            if (currentIdx % 6 === 0) fragment.appendChild(createDesktopNativeAdBlock());
        });
        subGrid.appendChild(fragment); libraryDisplayedCount = nextCount; subGrid.dataset.displayedCount = libraryDisplayedCount; updateLoadMoreVisibility();
    }
    isLoading = false;
}
function updateLoadMoreVisibility() { if (!document.getElementById('loadingIndicator')) return; if (libraryDisplayedCount < libraryData.length) document.getElementById('loadingIndicator').classList.remove('hidden'); else document.getElementById('loadingIndicator').classList.add('hidden'); }

// [UX: ভিডিও মোডাল ওপেন হলে হিস্ট্রি ডুপ্লিকেশন ছাড়া এবং ভিডিও modal close logic এর রিলিজ]
function openModal(id) {
    if(document.getElementById('mobileFab')) document.getElementById('mobileFab').classList.add('fab-hidden');
    savedScrollY = window.scrollY; const item = contentData.find(m => m.id === id); if (!item) return;

    const movieSlug = item.slug || generateMovieSlug(item.title); const newUrl = new URL('/' + movieSlug + '.html', window.location.origin);
    ...
    const modalTitleElem = document.getElementById('modalTitle'); const isSameMovie = modalTitleElem && modalTitleElem.innerText === item.title;
    currentItem = item; if(modalTitleElem) modalTitleElem.innerText = item.title;
    ...
    if (!seoContainer) {
        seoContainer = document.createElement('div'); seoContainer.id = 'modalSeoContent';
        ...
        if (adBottomWrapper) adBottomWrapper.parentNode.insertBefore(seoContainer, adBottomWrapper.nextSibling);
    }

    if (seoContainer) {
        let seoBodyContent = "";
        if (item.synopsis) seoBodyContent += `<div class="mt-6 text-left border-t border-white/5 pt-4 select-text"><b class="text-white text-sm md:text-base block mb-1">📖 Synopsis :</b><p class="text-gray-400 text-xs md:text-sm leading-relaxed">${item.synopsis}</p></div>`;
        if (item.movieHighlights) seoBodyContent += `<div class="mt-4 text-left select-text"><b class="text-white text-sm md:text-base block mb-1">✨ Movie Highlights :</b><p class="text-gray-400 text-xs md:text-sm leading-relaxed">${item.movieHighlights}</p></div>`;
        if (item.streamingRecommendation) seoBodyContent += `<div class="mt-4 text-left select-text"><b class="text-white text-sm md:text-base block mb-1">🎯 Streaming Recommendation :</b><p class="text-gray-400 text-xs md:text-sm leading-relaxed">${item.streamingRecommendation}</p></div>`;
        if (item.detailedPlotSummary) seoBodyContent += `<div class="mt-4 text-left select-text"><b class="text-white text-sm md:text-base block mb-1">🍿 Detailed Plot Summary :</b><p class="text-gray-400 text-[11px] md:text-xs leading-relaxed whitespace-pre-line bg-white/5 p-3 rounded-lg border border-white/10 select-text cursor-text">${item.detailedPlotSummary}</p></div>`;
        seoContainer.innerHTML = seoBodyContent;
    }

    downloadClickCount = 0; const downloadBtn = document.getElementById('mainDownloadBtn');
    if (downloadBtn) {
        ...
        if (downloadBtn.querySelector('.animate-shine-wave')) downloadBtn.querySelector('.animate-shine-wave').classList.remove('hidden');
    }

    if (item.episodes && item.episodes.length > 0) { if (!isSameMovie) currentEpisodeIndex = 0; } else { currentEpisodeIndex = null; }
    const seriesSec = document.getElementById('seriesSection'); const epList = document.getElementById('episodeList');
    if (item.episodes && seriesSec && epList) {
        seriesSec.classList.remove('hidden'); document.getElementById('seriesInfoText').innerText = item.seriesInfo; epList.innerHTML = '';
        item.episodes.forEach((ep, idx) => {
            const btn = document.createElement('button'); btn.className = `episode-btn px-6 py-3 rounded-lg bg-white/5 border border-white/10 text-[10px] font-black hover:bg-red-600 transition tracking-widest uppercase`;
            if (idx === (currentEpisodeIndex || 0)) btn.classList.add('active'); btn.innerText = ep.title; btn.onclick = () => playEpisode(idx, btn); epList.appendChild(btn);
        });
    } else if (seriesSec) { seriesSec.classList.add('hidden'); }
    ...
    if (actualVideoContainer) {
        actualVideoContainer.classList.remove('hidden'); const existingIframe = document.getElementById('videoIframe');
        if (!isSameMovie || !existingIframe || existingIframe.src === "" || existingIframe.src === "about:blank") {
    ...
    if (modal) {
        modal.classList.remove('hidden'); void modal.offsetWidth; modal.classList.add('active');
        setTimeout(() => { injectResponsiveAdNode(document.getElementById('modalAdTop')); injectResponsiveAdNode(document.getElementById('modalAdBottom')); }, 150);
    }
    document.body.style.position = 'fixed'; document.body.style.top = `-${savedScrollY}px`; document.body.style.width = '100%';
}

function handleDownloadClick() {
    if (!currentItem) return;
    if (downloadClickCount >= 3) {
        downloadClickCount = 0; const downloadBtn = document.getElementById('mainDownloadBtn'); document.getElementById('downloadBtnText').innerText = "Download";
        downloadBtn.classList.remove('from-gray-600', 'to-gray-800', 'border-gray-500', 'cursor-not-allowed', 'opacity-80'); downloadBtn.classList.add('from-[#2B2727]', 'to-[#2B2727]', 'border-[#E3DADA]', 'hover:scale-105');
        if (downloadBtn.querySelector('.animate-shine-wave')) downloadBtn.querySelector('.animate-shine-wave').classList.remove('hidden'); return;
    }
    downloadClickCount++;
    if (downloadClickCount === 1 || downloadClickCount === 2) { document.getElementById('downloadBtnText').innerText = downloadClickCount === 1 ? "Ready For Download" : "Download (Final Click)"; if (currentItem.downloadUrl1) window.open(currentItem.downloadUrl1, '_blank'); }
    else if (downloadClickCount === 3) {
        document.getElementById('downloadBtnText').innerText = "Link Expire"; const downloadBtn = document.getElementById('mainDownloadBtn');
        if (downloadBtn) { downloadBtn.classList.remove('from-[#2B2727]', 'to-[#2B2727]', 'border-[#E3DADA]', 'hover:scale-105'); downloadBtn.classList.add('!bg-none', '!bg-[#111]', '!border-white', '!text-white', 'cursor-not-allowed', 'opacity-80'); if (downloadBtn.querySelector('.animate-shine-wave')) downloadBtn.querySelector('.animate-shine-wave').classList.add('hidden'); }
        if (currentEpisodeIndex !== null && currentItem.episodes && currentItem.episodes[currentEpisodeIndex].downloadUrl) window.open(currentItem.episodes[currentEpisodeIndex].downloadUrl, '_blank'); else if (currentItem.downloadUrl2) window.open(currentItem.downloadUrl2, '_blank');
    }
}

function playEpisode(index, btnElement) {
    const episode = currentItem.episodes[index]; document.querySelectorAll('.episode-btn').forEach(b => b.classList.remove('active')); btnElement.classList.add('active'); let url = episode.embedUrl;
    const actualVideo = document.getElementById('actualVideo');
    if (actualVideo) {
        actualVideo.classList.remove('hidden'); const existingIframe = document.getElementById('videoIframe');
        if (!existingIframe || existingIframe.src !== url) actualVideo.innerHTML = `<iframe id="videoIframe" class="w-full h-full border-0 outline-none rounded-lg bg-black" src="${url}" frameborder="0" scrolling="no" marginwidth="0" marginheight="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share; fullscreen" allowfullscreen="true" webkitallowfullscreen="true" mozallowfullscreen="true"></iframe>`;
    }
    currentEpisodeIndex = index; downloadClickCount = 0; const downloadBtn = document.getElementById('mainDownloadBtn'); document.getElementById('downloadBtnText').innerText = "Download";
    downloadBtn.classList.remove('from-gray-600', 'to-gray-800', 'border-gray-500', 'cursor-not-allowed', 'opacity-80'); downloadBtn.classList.add('from-[#2B2727]', 'to-[#2B2727]', 'border-[#E3DADA]', 'hover:scale-105');
    if (downloadBtn.querySelector('.animate-shine-wave')) downloadBtn.querySelector('.animate-shine-wave').classList.remove('hidden');
}
...
function closeModal(triggerBack = true, explicitClose = false) {
    const modal = document.getElementById('movieModal'); if (!modal || modal.classList.contains('hidden')) return;
    if(document.getElementById('mobileFab')) document.getElementById('mobileFab').classList.remove('fab-hidden');
    isModalClosing = true; document.title = "MovieDakhi | Watch Free Movies & Web Series Online";
    if (!triggerBack) { modal.classList.add('hidden'); modal.classList.remove('active'); isModalClosing = false; }
    else { modal.classList.remove('active'); setTimeout(() => { modal.classList.add('hidden'); isModalClosing = false; }, 300); }
    setTimeout(() => { if (document.getElementById('actualVideo')) document.getElementById('actualVideo').innerHTML = ''; }, 300);
    document.body.style.position = ''; document.body.style.top = ''; document.body.style.width = ''; window.scrollTo(0, savedScrollY);
    if (triggerBack && window.history.state?.isModalOpen) window.history.back();
}

if (searchInput) {
    searchInput.addEventListener('focus', () => { if (!preSearchState && searchInput.value.trim().length === 0) preSearchState = { view: currentView, scrollY: window.scrollY, category: document.querySelector('#libraryFilters .category-pill.active')?.getAttribute('data-category') || 'all', displayedCount: libraryDisplayedCount }; });
    searchInput.addEventListener('input', debounce(() => {
        const rawQuery = searchInput.value; updateSearchUI();
        if (rawQuery.trim().length > 0) {
            if (!preSearchState) preSearchState = { view: currentView, scrollY: window.scrollY, category: document.querySelector('#libraryFilters .category-pill.active')?.getAttribute('data-category') || 'all', displayedCount: libraryDisplayedCount };
            if (currentView !== 'library') switchView('library'); initLibraryRender();
        } else { if (preSearchState) { switchView(preSearchState.view, preSearchState.category, 'replace', preSearchState.displayedCount, preSearchState.scrollY); preSearchState = null; } else { initLibraryRender(); } }
    }, 300));
    searchInput.addEventListener('blur', () => { setTimeout(() => { if ((document.getElementById('movieModal')?.classList.contains('active')) || (document.getElementById('categoryMenu')?.classList.contains('active'))) return; if (searchInput.value.trim().length === 0 && preSearchState) clearSearch(); }, 200); });
...
function showAnnouncement() {
    ...
    const isFacebookApp = /FBAN|FBAV|Ios/i.test(ua); const isUCBrowser = /UCBrowser|UCWEB|UCMini/i.test(ua); const isInstaApp = /Instagram/i.test(ua); const isAndroidWebviewApp = /wv|android.*version\/[0-9]/i.test(ua);
    const isOperaMini = ua.includes('Opera Mini') || ua.includes('OPR/'); const isTrappedApp = !isOperaMini && (isFacebookApp || isUCBrowser || isInstaApp || isAndroidWebviewApp);
    if (document.getElementById('browserWarningText') && isTrappedApp) {
        let msg = "This built-in browser <span class='text-white font-black'>Cannot Play or Download</span> movies. Tap below to use a real browser which you have!";
        if (isFacebookApp) msg = "Facebook browser <span class='text-white font-black'>Cannot Play or Download</span> movies. Tap below to use a real browser which you have!";
        else if (isUCBrowser) msg = "UC browser <span class='text-white font-black'>Cannot Play or Download</span> movies. Tap below to use a real browser which you have!";
        else if (isInstaApp) msg = "Instagram browser <span class='text-white font-black'>Cannot Play or Download</span> movies. Tap below to use a real browser which you have!";
        document.getElementById('browserWarningText').innerHTML = msg;
    }
    if (isTrappedApp) {
        if (document.getElementById('browserSuggestionBox')) document.getElementById('browserSuggestionBox').classList.remove('hidden');
        if (document.getElementById('announcementCloseBtnTop')) document.getElementById('announcementCloseBtnTop').classList.add('hidden');
        if (document.getElementById('popupBackdrop')) document.getElementById('popupBackdrop').onclick = null;
        if (document.getElementById('popupWelcomeText')) document.getElementById('popupWelcomeText').classList.add('hidden');
        if (document.getElementById('popupTelegramBtn')) document.getElementById('popupTelegramBtn').classList.add('hidden');
        if (document.getElementById('popupBoxContainer')) { document.getElementById('popupBoxContainer').classList.remove('p-5', 'md:p-8'); document.getElementById('popupBoxContainer').classList.add('p-4'); }
    } else {
        if (document.getElementById('browserSuggestionBox')) document.getElementById('browserSuggestionBox').classList.add('hidden');
        if (document.getElementById('announcementCloseBtnTop')) document.getElementById('announcementCloseBtnTop').classList.remove('hidden');
        if (document.getElementById('popupBackdrop')) document.getElementById('popupBackdrop').onclick = closeAnnouncement;
        if (document.getElementById('popupWelcomeText')) document.getElementById('popupWelcomeText').classList.remove('hidden');
        if (document.getElementById('popupTelegramBtn')) document.getElementById('popupTelegramBtn').classList.remove('hidden');
        if (document.getElementById('popupBoxContainer')) { document.getElementById('popupBoxContainer').classList.add('p-5', 'md:p-8'); document.getElementById('popupBoxContainer').classList.remove('p-4'); }
    }
    ...
    document.body.style.position = 'fixed'; document.body.style.top = `-${announcementScrollY}px`; document.body.style.width = '100%';
...
// 🚀 AUTOMATED DEEP LINK BREAKOUT SYSTEM (For Chrome, Edge, Brave, Opera, Firefox - Official & Beta)
function autoBreakoutToRealBrowser() {
    ...
    if (isAndroid) {
        // অ্যান্ড্রয়েডের ইউনিভার্সাল ব্রাউজার ইনটেন্ট। এটি ফোনে থাকা যেকোনো মেইন রিয়েল ব্রাউজার (Chrome, Beta, Edge, Brave, Opera, Firefox) দিয়ে escaping ও সিস্টেম ডিফল্ট অ্যাপ রান করতে বাধ্য করবে।
        ...
        // আইওএস ইন-অ্যাপ ব্রাউজার ব্রেকআউটের জন্য ক্রোম এবং এজ স্কিম ট্রিগার করা
...
function openInBrowser(browser) {
    autoBreakoutToRealBrowser(); // ম্যানুয়াল ক্লিকেও আমরা ফুল অটোব্রেকআউট ফায়ার করব
    showToast("Opening requested browser view...");
}

const uaCheck = navigator.userAgent || navigator.vendor || window.opera;
const isFBCheck = /FBAN|FBAV|Ios/i.test(uaCheck); const isUCCheck = /UCBrowser|UCWEB|UCMini/i.test(uaCheck); const isInstaCheck = /Instagram/i.test(uaCheck); const isAndroidWebviewCheck = /wv|android.*version\/[0-9]/i.test(uaCheck);
...
if (shouldShowPopup) {
    if (isTrappedCheck) {
         ...
         // 🎯 ম্যাজিক: ইন-অ্যাপ ব্রাউজার ডিটেক্ট হওয়া মাত্রই ১ মিলি-সেকেন্ডে অটোমেটিক ব্রেকআউট ইনটেন্ট ট্রিগার হবে!
